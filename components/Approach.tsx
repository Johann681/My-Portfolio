...
const Card = ({
  ...
  return (
    <div
      ...
      style={{
        // Sleek background transition and color change on hover
        background: hovered
          ? "linear-gradient(90deg, rgba(12,14,35,1) 0%, rgba(4,7,29,1) 100%)"
...
const AceternityIcon = ({ order }: { order: string }) => {
  return (
    <div>
      ...
      <button className="relative inline-flex overflow-hidden rounded-lg ">
      <span
          className="absolute inset-[-1000%] animate-[spin_2s_linear_infinite]
         bg-[conic-gradient(from_90deg_at_50%_50%,#E2CBFF_0%,#393BB2_50%,#E2CBFF_100%)]"
        />
        <span
          className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-lg bg-slate-950 px-5 py-2 text-purple backdrop-blur-3xl font-bold text-2xl"
        >
          {order}
        </span>
      </button>
